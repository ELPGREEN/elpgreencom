...
const countryRegulations: Record<string, CountryRegulation> = {
  ...
  Poland: {
    ...
    licenses: ["Pozwolenie zintegrowane", "Wpis do rejestru BDO", "Pozwolenie na budowę", "Wpis do CEIDG lub KRS"],
    laborRegulations: [
      "Kodeks pracy",
  ...
  Netherlands: {
    ...
    licenses: ["Omgevingsvergunning", "VIHB-registratie", "Inschrijving KvK", "Vergunning gevaarlijke stoffen"],
    laborRegulations: [
  ...
  Austria: {
    ...
    mainLaws: ["Abfallwirtschaftsgesetz (AWG 2002)", "Altreifenverordnung", "UVP-Gesetz 2000", "Gewerbeordnung (GewO)"],
    environmentalRequirements: [
    ...
    licenses: ["Gewerbeberechtigung", "Abfallbehandlererlaubnis", "Baubewilligung", "Firmenbucheintragung"],
    laborRegulations: [
  ...
  Switzerland: {
    ...
    licenses: ["Betriebsbewilligung (Kanton)", "Baubewilligung", "Handelsregistereintrag", "Abfallbewilligung"],
    laborRegulations: [
      "Arbeitsgesetz (ArG)",
  ...
  China: {
    ...
    licenses: ["营业执照 (再生资源回收经营)", "环评批复", "排污许可证", "安全生产许可证"],
    laborRegulations: ["劳动法、劳动合同法", "安全生产法", "职业病防治法", "工伤保险条例"],
  },
  India: {
  ...
  Japan: {
    ...
    licenses: ["産業廃棄物処理業許可 (収集運搬・処分)", "一般廃棄物処理業許可", "建築確認", "法人登記"],
    laborRegulations: ["労働安全衛生法", "労働基準法", "労働者災害補償保険法", "労働安全衛生規則"],
  },
  "South Korea": {
    ...
    licenses: ["폐기물처리업 허가 (종합/개별)", "사업자등록", "건축허가", "공장등록"],
    laborRegulations: ["산업안전보건법", "근로기준법", "산업재해보상보험법", "위험성평가 실시"],
  },
  Indonesia: {
    ...
    licenses: ["NIB (Nomor Induk Berusaha) via OSS", "Izin Usaha Industri", "Izin Lingkungan", "Pertek Limbah B3"],
    laborRegulations: [
  ...
  Thailand: {
    ...
    licenses: ["Factory License (DIW)", "Environmental license", "Company registration (DBD)", "VAT registration"],
    laborRegulations: [
  ...
  Philippines: {
    ...
    licenses: ["Environmental Compliance Certificate", "Business Permit (LGU)", "SEC Registration", "TSD Registration"],
    laborRegulations: [
  ...
  Turkey: {
    ...
    licenses: ["Çevre İzin ve Lisansı", "İşyeri Açma ve Çalışma Ruhsatı", "Ticaret Sicil Kaydı", "Yapı Ruhsatı"],
    laborRegulations: [
      "İş Kanunu (No. 4857)",
  ...
  Egypt: {
    ...
    licenses: ["Environmental Approval", "Industrial License (IDA)", "Commercial Registry", "Tax Card"],
    laborRegulations: [
...
serve(async (req) => {
  ...
  try {
    const { study, language = "en" } = (await req.json()) as { study: FeasibilityStudy; language?: string };
    ...
    const systemPrompt = `You are an expert industrial consultant specializing in OTR (Off-The-Road) tire recycling plants with 25+ years of experience in environmental regulations, industrial licensing, market conditions, and economic viability analysis.
...
You MUST provide an EXHAUSTIVE and COMPLETE analysis with NO TOKEN LIMITS - write as much as needed to cover everything. DO NOT truncate or abbreviate any section. Each section must be thoroughly analyzed.
...
CRITICAL INSTRUCTION: There is NO TOKEN LIMIT. Write a COMPLETE, EXHAUSTIVE analysis covering ALL sections. Aim for 4000-6000 words. Do NOT stop until all sections are thoroughly covered with actionable insights.`;
